...
class HTMLTextParser:
    ...
    def __init__(self):
        ...
        # Нужен ленивый захват, иначе хапает всё до конца строки
        # _replace_tags - во внешний файл настройки.
        # Или оставить здесь и выносить не регулярки, а текстовые пометки
    ...
    def generate_readability_text(self, graped_html_text_data, text_width: int = 80):
        ...
        text = self.__delete_tags_by_regex(text, self._all_tags_regex)
        # Очищаем случайно проскочившие blacklist
    ...
    def adjust_text_by_width(self, text, text_width: int = 80):
        ...
        for line in text.split("\n"):
            ...
            for word in line.split():
                # +1 - учет пробела
                potential_width = len(word) + current_width + 1
    ...
    def adjust_long_word(self, word, text_width):  # Это не умный переносчик.
        chunks = [word[i:i + text_width] for i in range(0, len(word),
                                                        text_width)]
        return chunks

    # Похоже намудрил
    def __replace_tag_by_local_part(self, text: str, regex, local_regex: str, replacement: str, replacement_empty):
        """replace html tags by its local part.
        Local part and tag are searching by regex and local_regex.
        replacement - string in format to use str.format func.
        replacement_empty - list of strings witch must to
    ...
    def __replace_tags_stuff(replace_to_empty_list, tag: str):
        """Replace to empty string all values from
         replace_to_empty_list in tag string"""
